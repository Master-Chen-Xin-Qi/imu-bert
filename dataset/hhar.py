...
DATASET_PATH = r'D:\\数据集\\hhar Activity recognition exp'
...
def preprocess_hhar(path, path_save, version, window_time=50, seq_len=40, jump=0):
    ...
    data = []
    data_temp = []
    while time_index[0] < len(accs)/1000 and time_index[1] < len(gyros):
        print(time_index[0])
        # 每十个数据取一次均值
    ...
    data_new, label_new = separate_data_label(data_raw)  # data_new返回数据大小为 n*120*6 6表示三轴加速度计，三轴陀螺仪;
    # label_new返回 n*120*3 分别是：用户，机型，状态
