...
def split_last(x, shape):
    ...
    # x.view(x的前两个维度，x的最后一个维度分成head*width)
...
class LIBERTDataset4Pretrain(Dataset):
    ...
    def __getitem__(self, index):
        instance = self.data[index]
        for proc in self.pipeline:
            instance = proc(instance)  # pipeline中的类都是__call__的
...
def handle_argv(target, config_train, prefix):
    ...
    parser.add_argument('-dataset', type=str, help='Dataset name', choices=['hhar', 'motion', 'uci', 'shoaib'], default='hhar')
    parser.add_argument('-dataset_version',  type=str, help='Dataset version', choices=['10_100', '20_120'], default='20_120')
    parser.add_argument('-g', '--gpu', type=str, default=None, help='Set specific GPU')
    parser.add_argument('-f', '--model_file', type=str, default=None, help='Pretrain model file')  # 把None改成了model
    ...
    parser.add_argument('-l', '--label_index', type=int, default=2,    # 原来为-1，classifier.py时改为了2
                        help='Label Index')
    parser.add_argument('-s', '--save_model', type=str, default='test',
